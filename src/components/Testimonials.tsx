...
export default function Testimonials() {
  return (
    <section className="bg-[#3D2B3F] py-20 text-white">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-3xl font-bold sm:text-4xl">Vad våra kunder säger</h2>
          <p className="mt-4 text-white/70">Ordagrant utdrag från Bokadirekt.</p>
        </div>
          ...
          {business.testimonials.map((review, index) => (
            <figure key={index} className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
              ...
              <figcaption className="mt-4 text-xs text-white/60">
                — {review.author} ·{" "}
                <a href={review.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline hover:text-white">{review.source}</a>
